...
(
	...
	kubectl create secret generic ${PRODUCT}-daemon-certs -n buildkit -o yaml --from-file=./daemon >${PRODUCT}-daemon-certs.yaml
	kubectl create secret generic ${PRODUCT}-client-certs -n buildkit -o yaml --from-file=./client >${PRODUCT}-client-certs.yaml
)
